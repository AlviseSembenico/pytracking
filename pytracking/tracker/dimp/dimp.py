...
class DiMP(BaseTracker):
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        if num_iter > 0:
            ...
            temp_samples = torch.cat((samples, train_x))
            temp_target_boxes = torch.cat((target_boxes, target_box[None, ...].to(target_boxes.device)))
            temp_sample_weights = torch.cat((sample_weights, torch.ones(1).to(sample_weights.device) / sample_weights.shape[0]))
            temp_sample_weights /= temp_sample_weights.sum()

            # Run the filter optimizer module
            prev_target_filter = self.target_filter
            self.target_filter, _, losses = self.net.classifier.filter_optimizer(self.target_filter,
                                                                                 num_iter=num_iter, feat=temp_samples,
                                                                                 bb=temp_target_boxes,
                                                                                 sample_weight=temp_sample_weights,
                                                                                 compute_losses=plot_loss)
            diff = torch.abs(self.target_filter - prev_target_filter).sum()
            diff.backward()
            grad_change = train_x.grad.norm()

            if grad_change <= self.params.grad_lb:
                if hard_negative_flag or self.frame_num % self.params.get('train_sample_interval', 1) == 0:
                    self.update_memory(TensorList([train_x.detach()]), target_box, learning_rate)
            else:
                self.target_filter = prev_target_filter

            self.target_filter.detach_()
